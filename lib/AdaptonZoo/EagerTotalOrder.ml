...
module T = struct
  ...
  type 'a thunk = { (* 3 + 16 = 19 words *)
                    id : int; mutable value : 'a; meta : meta }
  ...
  and meta = {
    (* 5 + 5 + 5 = 15 words (not including closures of evaluate and unmemo as well as WeakBucket.t) *)
    mutable evaluate : unit -> unit;
    mutable unmemo : unit -> unit;
    mutable start_timestamp : TotalOrder.t;
    (* for const thunks, {start,end}_timestamp == TotalOrder.null and evaluate == nop *)
  ...
  let force m =
    ...
    (match !eager_stack with dependent :: _ -> WeakBucket.add m.meta.dependents dependent | [] -> ());
    m.value
end
...
struct
  ...
  let make_evaluate m f () = update m (evaluate_meta m.meta f)
  ...
  include MemoN.Make (struct
    ...
    let memo (type a) (module A : Hashtbl.SeededHashedType with type t = a) f =
      ...
      let rec memo x =
        ...
        | Some m
          when TotalOrder.is_valid m.meta.start_timestamp
