...
module T : sig
  ...
  let rec null_parent =
    { parent_label = min_int; parent_next = null_parent; parent_prev = null_parent; front = null; back = null }

  and null = { label = min_int; parent = null_parent; prev = null; next = null; invalidator = nop }
  ...
  let create () =
    let rec ts =
      {
        label = 0;
        parent = { parent_label = 0; parent_next = null_parent; parent_prev = null_parent; front = ts; back = ts };
        prev = null;
        next = null;
        invalidator = nop;
      }
  ...
  let add_next ts =
    ...
    (if ts.label == ts'.label then
     ...
     let rec rebalance label parent prev next =
       if label < end_label then (
         next.label <- label;
         next.parent <- parent;
         if next.next != null then rebalance (label + gap_size) parent next next.next else parent.back <- next)
       else (
         (* add a new parent *)
         parent.back <- prev;
         prev.next <- null;
         ...
         (if parent.parent_label == parent'.parent_label then
          ...
          let rec expand lower upper count mask tau =
            ...
            let rec expand_lower lower count =
              if lower.parent_prev != null_parent then
                let lower' = lower.parent_prev in
                if lower'.parent_label >= lo_label then expand_lower lower' (count + 1) else (lower, count)
              else (
                if lower.parent_label != lo_label then lower.parent_label <- lo_label;
                (lower, count))
            in
            let rec expand_upper upper count =
              if upper.parent_next != null_parent then
                let upper' = upper.parent_next in
                if upper'.parent_label <= hi_label then expand_upper upper' (count + 1) else (upper, count)
              else (
                if upper.parent_label != hi_label then upper.parent_label <- hi_label;
                (upper, count))
            in
            ...
            else (lower, upper, lo_label, (mask + 1) / count)
          in
          ...
          let rec rebalance parent label =
            ...
            if parent != upper && parent.parent_next != null_parent then rebalance parent.parent_next (label + delta)
          in
    ...
    if inclusive then (
      let remove ts =
        if ts.prev == null then
          if ts.next == null then (
            if ts.parent.parent_next != null_parent then ts.parent.parent_next.parent_prev <- ts.parent.parent_prev;
            if ts.parent.parent_prev != null_parent then ts.parent.parent_prev.parent_next <- ts.parent.parent_next;
            invalidate_parent ts.parent)
          else (
            ts.next.prev <- null;
            ts.parent.front <- ts.next;
            invalidate ts)
        else if ts.next == null then (
          ts.prev.next <- null;
