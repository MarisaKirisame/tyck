...
module Make (M : MemoNType) = struct
  ...
  let memo2 (type a) (module A : Hashtbl.SeededHashedType with type t = a) (type b)
      (module B : Hashtbl.SeededHashedType with type t = b) f =
    let memo =
      M.memo
        (module struct
          type t = A.t * B.t
        ...
        (fun memo (a, b) -> f (fun a b -> memo (a, b)) a b)
    in
    fun a b -> memo (a, b)

  (** Create memoizing constructor of arity 3. *)
  let memo3 (type a) (module A : Hashtbl.SeededHashedType with type t = a) (type b)
      (module B : Hashtbl.SeededHashedType with type t = b) (type c)
      (module C : Hashtbl.SeededHashedType with type t = c) f =
    let memo =
      M.memo
        (module struct
          type t = A.t * B.t * C.t
        ...
        (fun memo (a, b, c) -> f (fun a b c -> memo (a, b, c)) a b c)
    in
    fun a b c -> memo (a, b, c)

  (** Create memoizing constructor of arity 4. *)
  let memo4 (type a) (module A : Hashtbl.SeededHashedType with type t = a) (type b)
      (module B : Hashtbl.SeededHashedType with type t = b) (type c)
      (module C : Hashtbl.SeededHashedType with type t = c) (type d)
      (module D : Hashtbl.SeededHashedType with type t = d) f =
    let memo =
      M.memo
        (module struct
        ...
        (fun memo (a, b, c, d) -> f (fun a b c d -> memo (a, b, c, d)) a b c d)
    in
