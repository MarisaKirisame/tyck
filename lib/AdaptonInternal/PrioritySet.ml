...
module Make (O : Set.OrderedType) = struct
  ...
  let rec add queue x =
    ...
    | Node (value, left, right) ->
        ...
        if dir == 0 then false else if dir < 0 then add left x else add right x
    | Null ->
  ...
  let rec pop queue =
    match !queue with
    | Node (_, ({ contents = Node _ } as left), _) -> pop left
    | Node (value, { contents = Null }, right) ->
        queue := !right;
        value
    | Null -> raise Empty

  let rec remove queue x =
    match !queue with
    | Node (value, left, right) ->
        let dir = O.compare x value in
        if dir == 0 then (
          ...
          true)
        else if dir < 0 then remove left x
        else remove right x
    | Null -> false
end
