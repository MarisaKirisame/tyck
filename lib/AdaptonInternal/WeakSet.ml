...
module Make (H : Hashtbl.HashedType) = struct
  ...
  type t = { mutable extent : int; mutable array : H.t Weak.t }
  type data = H.t
  ...
  let fold fn xs acc =
    ...
    if is_array xs then (
      ...
      for i = 0 to Weak.length xs.array - 1 do
        match Weak.get xs.array i with Some x -> acc := fn x !acc | None -> ()
      done;
    !acc
  ...
  let rec merge xs x =
    let resize () =
      ...
      if is_array xs then
        ...
        for i = 0 to old_size - 1 do
          match Weak.get old_array i with Some x -> ignore (merge xs x) | None -> ()
        done
    in
    if is_array xs then (
      ...
      let x'opt =
        fold
          (fun x' x'opt -> match x'opt with Some _ -> x'opt | None -> if H.equal x' x then Some x' else None)
          xs None
      in
      match x'opt with
      | Some x -> x
      ...
      let rec find j result =
        if j <= xs.extent then
          ...
          match result with
          | Some (j, k) ->
              xs.extent <- max j xs.extent;
              Weak.set xs.array k (Some x);
              x
          | None ->
              let rec find j =
                if j < window then (
